...
export const formValidationSchema = [
	...
	z.object({
		selectedProductPrices: z
			...
			.refine(
				(items) =>
					items.every((item) => item.product_id && item.price_id),
	...
	z.object({
		existingConditions: z.enum(['yes', 'no'], {
			message: 'Select an option',
		}),
	...
	z.object({
		autoimmuneDisorders: z.enum(['yes', 'no'], {
			message: 'Select an option',
		}),
	...
	z.object({
		...
		recentHealthChanges: z.enum(['yes', 'no'], {
			message: 'Select an option',
		}),
	...
	z.object({
		injectablesConcerns: z.enum(['yes', 'no'], {
			message: 'Select an option',
		}),
